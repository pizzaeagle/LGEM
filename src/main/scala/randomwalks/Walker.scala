...
import mm.graph.embeddings.node2vec.{
  CalculateAliases, CollectNodesNeighbourhood, CreateTripletsWithNeighbourhood, RandomWalk
}
...
object Walker {
  def walk(g: Graph, walkLength: Int, numWalks: Int, p: Int = 1, q: Int = 1)(
    ...
    walks
      .withColumn("walk", array(cols.map(col(_).cast(LongType)): _*))
      .drop(cols: _*)
      .as[RandomWalk]
  }
}
